...
//  AnimationChallange
//
//  Created by Waris on 2023/01/16.
//
...
struct Home: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            VStack(spacing: 10){
                Text("Budgets")
                    .font(.custom(Uyghur, size: 30, relativeTo: .title))
                ...
                //MARK: Making it above the ScrollView
                CardView()
                    .zIndex(1)
            ...
            //MARK: Moving view up byu negative padding
            .padding(.top,expandMenu ? 30 : -130)
            //MARK: Dimming Content
            .overlay{
                Rectangle()
                    .fill(.black)
                    .opacity(dimContent ?  0.4 : 0)
                    .ignoresSafeArea()
            }
        }
    ...
    func animateMenu(){
        if expandMenu {
            //MARK: Closing With little speed
            withAnimation(.easeInOut(duration: 0.25)){
                dimContent = false
            }
            //MARK: Dimming Content Little Later
    ...
    func CardView()-> some View{
        HStack{
            VStack(alignment: .leading, spacing: 10) {
                Text("Total")
                    .font(.custom(Uyghur, size: 20, relativeTo: .body))
                
                Text("$10,25")
                    .font(.custom(UyghurTom, size: 28, relativeTo: .largeTitle))
                    .fontWeight(.medium)
        ...
        HStack(spacing: 12){
            ...
            VStack(alignment: .leading, spacing: 4){
                Text(expense.product)
                    .font(.custom(Uyghur, size: 16, relativeTo: .body))
                Text(expense.spendType)
                    .font(.custom(Uyghur, size: 12, relativeTo: .caption))
            }
            .frame(maxWidth: .infinity, alignment: .leading)
            
            Text(expense.amountSpent)
                .font(.custom(Uyghur, size: 18, relativeTo: .title3))
                .fontWeight(.medium)
                .foregroundColor(Color("Blue"))
        }
        .padding(10)
